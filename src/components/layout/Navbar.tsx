...
import { Menu, X, Box, ChevronDown, Rocket, Zap, BarChart3, Globe, Shield, MessageSquare, HelpCircle } from 'lucide-react';
...
const products = [
  { 
    ...
    items: [
      { name: 'Custom Web Development', description: 'Bespoke architectures built for extreme performance.', href: '/services/custom-web' },
      { name: 'WordPress Development', description: 'Enterprise-grade WordPress systems that scale.', href: '/services/wordpress' },
      { name: 'Shopify Store Development', description: 'High-converting headless commerce solutions.', href: '/services/shopify' },
    ]
  },
  { 
    category: 'Marketing', 
    items: [
      { name: 'Search Engine Optimization', description: 'Semantic search dominance for consistent traffic.', href: '/services/seo' },
      { name: 'Performance Marketing', description: 'ROI-focused Meta & Google Ads optimization.', href: '/services/marketing' },
    ]
  },
  { 
    category: 'Automation', 
    items: [
      { name: 'n8n Automation', description: 'Custom workflow architectures for maximum efficiency.', href: '/automations' },
    ]
  },
];
...
export const Navbar = () => {
  ...
  return (
    <div className="fixed top-6 left-0 right-0 z-[100] flex justify-center w-full pointer-events-none px-4">
      <motion.nav
        ...
        className="max-w-7xl w-full flex items-center justify-between px-6 h-18 rounded-full border border-white/10 shadow-2xl pointer-events-auto relative transition-all"
      >
        <Link to="/" className="flex items-center gap-2 group cursor-pointer z-[101]">
          <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center shadow-glow group-hover:shadow-strong-glow transition-all duration-300">
          ...
          >
            ...
            <AnimatePresence>
              {activeMegaMenu === 'services' && (
                ...
                >
                  <div className="rounded-3xl p-10 shadow-2xl border border-white/10 shadow-primary/5 bg-black">
                    <div className="grid grid-cols-3 gap-10">
                      {products.map((cat) => (
                        <div key={cat.category}>
                          <h4 className="text-[10px] uppercase font-black tracking-[0.2em] text-white/50 mb-6 px-2">{cat.category}</h4>
                          <div className="space-y-3">
                            {cat.items.map((item) => (
                              ...
                              >
                                <div className="font-bold text-sm text-white mb-1 group-hover:text-white transition-colors">{item.name}</div>
                                <div className="text-[11px] text-white/40 leading-relaxed line-clamp-2">{item.description}</div>
                              </Link>
                            ))}
                          </div>
                        </div>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="lg:hidden absolute top-[110%] left-0 right-0 glass p-6 flex flex-col gap-2 rounded-3xl border border-white/10 shadow-2xl origin-top overflow-y-auto max-h-[85vh] bg-background/95 backdrop-blur-3xl z-[150] pointer-events-auto"
            >
              <Link
                to="/"
                className="p-4 rounded-2xl hover:bg-primary/10 transition-colors text-text-primary font-bold text-lg"
                  ...
                  <div className="space-y-1">
                    {cat.items.map((item) => (
                      <Link
                        key={item.name}
                        to={item.href}
                        className="flex flex-col p-4 rounded-2xl hover:bg-primary/10 transition-colors"
                        onClick={() => setIsOpen(false)}
                      >
